...
the bake runs in a single process-wide web worker shared by every donut on the page. `donut-cache.ts` keys bakes by the full config, so N identical donuts share one bake, one frame array, one worker — refcounted, evicted when the last instance unmounts, worker terminated when no bakes remain. if the worker fails to load or never answers within 1s (blocked in some bundlers), a chunked main-thread bake fills the array over idle slices instead. sampling density adapts to the projected circumference and to device tier (`hardwareConcurrency`/`deviceMemory`); low-end devices get coarser sampling and a ~30fps frame budget.

**the element is never empty, and never stutters.** the first frame is painted synchronously on mount, before the first animation frame — the bake takes hundreds of milliseconds to land, and a donut placed inside an entrance animation would otherwise fade in as an empty box and pop to full opacity when the frames arrived. after that, playback only ever shows *precomputed* frames: when the next one isn't baked yet it holds on the one it is showing rather than computing live, because live computation during playback is exactly what makes the spin stutter while the bake lands. the donut is therefore still before it spins, never absent before it appears.
...
the overlay is a three-row flex column — an empty row, the hero+lines box, the skip row — and that shape is load-bearing on a short viewport. the skip button is *in flow*, never `fixed`: as a fixed element it took up no layout space, so a centred column taller than the viewport simply grew down over it and the last line rendered on top of the word "skip". the empty first row is its twin, so the content between the two stays centred on the viewport rather than on the space above the button. the hero+lines box is `min-h-0` and the hero inside it is the one thing allowed to give way, so when the viewport is shorter than the hero wants — an in-app browser's sheet leaves ~550px where a phone's own browser gives ~850 — the art is cropped evenly rather than the skip row being pushed off the bottom. **size a tall `hero` against the short case**: give it a `svh`/`vh` term (justin06lee.dev's donut hero is sized `text-[min(2.2vw,1.8svh,0.75rem)]` — a vw term for a narrow screen, an svh term for a short one, and a rem cap so it never grows past where it started) rather than a fixed size, so it shrinks instead of relying on that crop.
